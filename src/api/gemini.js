...
### カラムレイアウト（最重要）
- **画面を右から左に3～5個の縦書きカラムに分割すること**
- **最も重要な情報を右端のカラムに配置すること**
- **各カラム内では、テキストを上から下に縦書きで配置すること**
- **各カードは1つの独立した縦書きブロックとして扱うこと**
- **タグ（【】内のテキスト）と本文を同じカラム内に配置すること**

### 文字サイズと配置
- **右端のカラム：最大サイズ（最重要情報）**
- **中央のカラム：中サイズ（重要情報）**
- **左端のカラム：小サイズ（補足情報）**
- ヘッダーは画面上部に横書きで配置しても良い
- フッターは画面下部に横書きで配置しても良い

### 意味のブロックを保持
- **タグ（例：【速報】）とその内容は必ず同じカラム内に配置すること**
- **関連する情報を分断しないこと**
- **各カードの背景色で意味のまとまりを示すこと**

## YAML仕様
${yamlContent}

この仕様に従って、日本の週刊誌の表紙のような、16:9の横長フォーマットで縦書きレイアウトの高品質なサムネイル画像を生成してください。テキストは全て日本語で、縦書きで読みやすく配置してください。`
  }
